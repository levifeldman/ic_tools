...
class IC {
    ...
    IC(this.canisterIdText) : canisterIdBlob= icidtextasablob(canisterIdText), canisterbaseurl= icbaseurl + '/api/v2/canister/$canisterIdText/';
    ...
    Future<dynamic> callCanister({required String calltype, required String methodName}) async {
        ...
        if (calltype == 'call') {
            ...
            while (!['replied','rejected','done'].contains(callstatus) && c <= 15) {
                ...
                pathsvalues = await getSystemStatePaths( 
                    paths: [
                        ['time'],
            ...
            if (callstatus=='replied') {
                canistersponse = candidsponsebytesasadarttype(pathsvalues[2]);
            }
        } 
        else 
        if (calltype == 'query') {
    ...
    Future<List> getSystemStatePaths({required List<List<dynamic>> paths, http.Client? httpclient}) async {        
...
void verifycertificate(Map certificate) {
    ...
    if (certificate.containsKey('delegation')) {
        verifycertificate(certificate['delegation']['certificate']);
        derKey = lookuppathvalueinaniccertificatetree(certificate['delegation']['certificate']['tree'], ['subnet', certificate['delegation']['subnet_id'], 'public_key']);
    } else {
        derKey = icrootkey; }
    Uint8List blskey = derkeyasablskey(derKey);
    // print(certificate['signature'].runtimeType);
...
Uint8List createcandidparams(dynamic darttypeforthecandid) {
    return Uint8List.fromList(utf8.encode('DIDL'));
}

dynamic candidsponsebytesasadarttype(Uint8List candidbytes) {
    return candidbytes;
}
